...
func TestCompile_Secrets(t *testing.T) {
	...
	if got, want := ir.Server.Hostname, "localhost:22"; got != want {
		t.Errorf("Want server host %s, got %s", want, got)
	}
	if got, want := ir.Server.Username, "root"; got != want {
		t.Errorf("Want server user %s, got %s", want, got)
	}
	if got, want := ir.Server.Password, "password"; got != want {
		t.Errorf("Want server password %s, got %s", want, got)
	}
	if got, want := ir.Server.SSHKey, "-----BEGIN RSA PRIVATE KEY-----"; got != want {
		t.Errorf("Want server ssk_key %s, got %s", want, got)
	}
}
